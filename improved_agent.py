...
def create_improved_agent(
    ...
    Args:
        ...
        fast_mode: If True, enables performance optimizations for speed
    ...
    # Create callbacks list - optimize based on fast_mode
    callbacks = []
    
    # Always include GNOME hot corner prevention - MUST be first to intercept move commands
    callbacks.append(GnomeHotCornerPrevention(hot_corner_threshold=50))
    
    if fast_mode:
        # Fast mode: Only essential callbacks for speed
        logging.info("🚀 Fast mode enabled - using minimal callbacks for maximum speed")
        
        # Only essential callbacks
        callbacks.extend([
            # Image retention for memory efficiency (fewer images in fast mode)
            ImageRetentionCallback(only_n_most_recent_images=3),
            
            # Budget management (higher budget for fast mode)
            BudgetManagerCallback(max_budget=10.0),
        ])
    else:
        # Full mode: All callbacks for comprehensive behavior
        logging.info("🔧 Full mode - using all callbacks for comprehensive behavior")
    ...
    # Add system prompt for OSWorld tasks (optimized based on mode)
    if fast_mode:
        # Streamlined instructions for fast mode
        osworld_instructions = (
            "You are an expert computer operator completing OSWorld benchmark tasks QUICKLY. "
            "SPEED IS CRITICAL - work efficiently and decisively. "
            
            "FAST EXECUTION RULES: "
            "1. ALWAYS start with screenshot to see current state. "
            "2. Act decisively - don't overthink, just execute based on what you see. "
            "3. Use EXACT coordinates from visible elements in screenshot. "
            "4. NEVER click at (0,0) - always target visible UI elements. "
            "5. Don't wait unnecessarily - the system handles delays automatically. "
            "6. Clear text fields with Ctrl+A before typing. "
            "7. If Activities overlay appears, press Escape immediately and retry. "
            
            "SPEED OPTIMIZATIONS: "
            "8. Skip verification screenshots unless absolutely necessary. "
            "9. Use keyboard shortcuts when faster than mouse (Ctrl+S, Alt+F4). "
            "10. Don't repeat failed actions - try different approach immediately. "
            "11. Be direct and efficient - avoid unnecessary steps. "
            "12. State completion clearly when task is done."
        )
    else:
        # Full instructions for comprehensive mode
        osworld_instructions = (
    ...
    # Create the agent with optimized settings based on mode
    if fast_mode:
        # Fast mode settings - prioritize speed over comprehensive logging
        agent = ComputerAgent(
            model=model,
            tools=tools,
            callbacks=callbacks,
            verbosity=logging.WARNING,  # Reduce logging overhead
            only_n_most_recent_images=2,  # Keep fewer images for speed
            use_prompt_caching=True,
            max_retries=1,  # Fewer retries for speed
            screenshot_delay=0.1,  # Much faster screenshot delay
            trajectory_dir=None  # Skip trajectory saving for speed
        )
    else:
        # Full mode settings - comprehensive behavior
        agent = ComputerAgent(
            model=model,
            tools=tools,
            callbacks=callbacks,
            ...
            screenshot_delay=0.3,  # Reduced from default 0.5s
    ...
    agent.get_stats = lambda: {
        ...
        "tools": len(tools),
        "fast_mode": fast_mode,
        "task_planning": "disabled" if fast_mode else "enabled",
        "knowledge_base": "disabled" if fast_mode else "enabled",
        "gnome_protection": "programmatic+prompt",
        "hot_corner_prevention": "enabled",
        "screenshot_delay": 0.1 if fast_mode else 0.3
    }
